...
const pageUrl = pathToFileURL(
  path.resolve("/Users/rob/projects/basement/build/ticket-20-touch-check/index.html")
).href;
...
// ---------- Desktop: 1440x900, mouse ----------
{
  ...
  const buttons = page.locator(".chart-frame").first().locator(".chart-actions button");
  ...
  check(
    "desktop: rainfall hover value shows mm",
    rainLegend.skipped || rainLegend.cells.some((cell) => / mm$/.test(cell)),
  ...
  await page.screenshot({
    path: `${screenshotDir}/ticket-20-touch-desktop.png`,
    fullPage: false,
  });
  await context.close();
}
...
// ---------- Mobile: 390x844, touch ----------
{
  ...
  const chartTop = page.locator(".chart-frame").first();
  ...
  await page.screenshot({
    path: `${screenshotDir}/ticket-20-touch-mobile.png`,
    fullPage: false,
  });
  await context.close();
}
