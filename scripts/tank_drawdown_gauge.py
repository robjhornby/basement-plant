...
curated dataset plus the tank-full events you log in data/basement_events.csv, then reports:
...
SNAPSHOT_DIRECTORY = REPOSITORY_ROOT / "data" / "parquet-2026-08-07"  # local `aws s3 cp` of parquet/ from R2
EVENTS_CSV = REPOSITORY_ROOT / "data" / "basement_events.csv"         # your hand-logged tank-full times
...
PROMINENCE_MINIMUM_RELATIVE_HUMIDITY = 0.8       # relative-humidity points a cycle must stand out by to count
COLLAPSE_MINUTES = 15                            # extrema closer than this collapse into one

# Gauge / live estimate
DRAWDOWN_METRIC = "absolute_humidity"            # "absolute_humidity" (physical, placement-robust) or "relative_humidity"
...
# Load
# ---------------------------------------------------------------------------------------------
def load_basement() -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Return (timestamps, minute_index, relative_humidity, absolute_humidity) for the Basement since install."""
    connection = duckdb.connect()
    rows = connection.execute(
        f"""
        select epoch(timestamp)::bigint, relative_humidity_pct, absolute_humidity_g_m3
        from read_parquet('{SNAPSHOT_DIRECTORY}/sensor_readings/**/*.parquet', hive_partitioning=true)
        where location = 'Basement'
          and timestamp >= timestamp '{DEHUMIDIFIER_INSTALLED_AT:%Y-%m-%d %H:%M}'
        order by timestamp
        """
    ).fetchall()
...
def load_tank_full_events() -> list[datetime]:
    """Tank-full timestamps from the events CSV (any row whose text mentions a full tank)."""
    events = []
    with EVENTS_CSV.open() as events_file:
        for row in csv.DictReader(events_file):
            if "tank full" in row["Event"].lower():
                events.append(datetime.strptime(row["Time"], "%Y/%m/%d %H:%M:%S"))
...
def rolling_median(values: np.ndarray, window: int) -> np.ndarray:
    ...
    return np.array([np.median(values[max(0, i - half_window):i + half_window + 1]) for i in range(len(values))])
...
def find_extrema(values: np.ndarray, kind: int) -> list[int]:
    ...
    for i in range(len(values)):
        ...
        if not len(left) or not len(right):
            continue
        prominence = (min(left.max(), right.max()) - values[i]) if kind == -1 else (values[i] - max(left.min(), right.min()))
    ...
    for i in raw_indices:
        if collapsed and i - collapsed[-1] < COLLAPSE_MINUTES:
            is_better = values[i] < values[collapsed[-1]] if kind == -1 else values[i] > values[collapsed[-1]]
            if is_better:
                collapsed[-1] = i
        else:
            collapsed.append(i)
...
class Analysis:
    ...
    def __init__(self) -> None:
        self.timestamps, self.minute_index, self.relative_humidity, self.absolute_humidity = load_basement()
        self.full_events = load_tank_full_events()
        self.smoothed_relative_humidity = rolling_median(self.relative_humidity, SMOOTH_WINDOW_MINUTES)
        ...
        self.drawdown_metric = self.absolute_humidity if DRAWDOWN_METRIC == "absolute_humidity" else self.relative_humidity
        ...
        for trough_index in self.troughs:
            ...
            if peak_index is not None:
                drawdown = max(0.0, self.drawdown_metric[peak_index] - self.drawdown_metric[trough_index])
    ...
    def _preceding_peak(self, trough_index: int) -> int | None:
        near = [peak for peak in self.peaks
                if peak < trough_index and self.minute_index[trough_index] - self.minute_index[peak] < 120]
    ...
    def _resume_after(self, full: datetime) -> np.datetime64:
        ...
        for trough_index in self.troughs:
            if self.minute_index[trough_index] <= full_minute + 30:
                continue
            window = [other for other in self.troughs
                      if self.minute_index[trough_index] <= self.minute_index[other] <= self.minute_index[trough_index] + RESUME_WINDOW_MINUTES]
    ...
    def dose(self, start_minute: int, end_minute: int) -> float:
        ...
        return float(self.cycle_drawdowns[(self.cycle_minutes >= start_minute) & (self.cycle_minutes <= end_minute)].sum())

    def interval_bounds(self, tank_index: int) -> tuple[int, int]:
        start = np.datetime64(DEHUMIDIFIER_INSTALLED_AT) if tank_index == 0 else self.emptied[tank_index - 1]
    ...
    def completed_tanks(self) -> list[tuple[float, float]]:
        ...
        for tank_index in range(len(self.full_events)):
            ...
            results.append((self.dose(start_minute, end_minute), (end_minute - start_minute) / 1440))
        return results
...
def main() -> None:
    if not SNAPSHOT_DIRECTORY.exists():
        raise SystemExit(
            ...
            "  aws s3 cp s3://$R2_BUCKET/parquet/ <dir>/ --recursive --endpoint-url $R2_ENDPOINT_URL"
        )
    analysis = Analysis()
    ...
    print(f"data: Basement {str(analysis.timestamps[0])[:16]} -> {str(analysis.timestamps[-1])[:16]} "
    ...
    gauges = {
        f"per-cycle {metric_label}-drawdown sum":
            np.array([analysis.dose(*analysis.interval_bounds(tank_index)) for tank_index in range(len(analysis.full_events))]),
        "cycle count":
            np.array([((analysis.cycle_minutes >= analysis.interval_bounds(tank_index)[0])
                       & (analysis.cycle_minutes <= analysis.interval_bounds(tank_index)[1])).sum()
    ...
    for tank_index, (dose, days) in enumerate(tanks):
        start = DEHUMIDIFIER_INSTALLED_AT if tank_index == 0 else analysis.emptied[tank_index - 1]
        print(f"   {tank_index+1}   | {str(start)[:16]} -> {analysis.full_events[tank_index]:%Y-%m-%d %H:%M} | {days:4.2f} | {dose:5.1f} | {dose/days:5.2f}")
    print(f"  DOSE_PER_TANK = {calibration:.1f}   scatter = +/-{tanks[:,0].std()/calibration*100:.0f}%")
    ...
    recent_drawdowns = analysis.cycle_drawdowns[(analysis.cycle_minutes >= now - RECENT_WINDOW_DAYS * 1440) & (analysis.cycle_minutes <= now)]
    drawdown_per_cycle = float(np.median(recent_drawdowns)) if len(recent_drawdowns) else float(np.median(analysis.cycle_drawdowns))
    cycles_remaining = remaining / drawdown_per_cycle if drawdown_per_cycle else float("inf")
    recent_rate = analysis.dose(now - RECENT_WINDOW_DAYS * 1440, now) / RECENT_WINDOW_DAYS  # dose/day
    ...
    print(f"  fraction full        {fraction_full*100:.0f}%  (~{fraction_full*TANK_CAPACITY_LITRES:.1f} of {TANK_CAPACITY_LITRES} L)")
    print(f"  cycles remaining     ~{cycles_remaining:.0f}")
    print(f"  time remaining       ~{days_remaining:.1f} d  (recent {RECENT_WINDOW_DAYS}d rate = {recent_rate:.1f} dose/day)")
    print(f"                       range +/-{scatter_fraction*100:.0f}% -> "
    ...
    for tank_index in range(len(analysis.full_events)):
        ...
        calibration_excluding_tank = np.mean([tanks[other, 0] for other in range(len(tanks)) if other != tank_index])
        rate = dose_at_midpoint / max((midpoint_minute - start_minute) / 1440, 1e-9)
        gauge_days = (midpoint_minute - start_minute) / 1440 + max(0.0, calibration_excluding_tank - dose_at_midpoint) / max(rate, 1e-9)
        naive_days = np.mean([tanks[other, 1] for other in range(len(tanks)) if other != tank_index])
        gauge_errors.append(gauge_days - true_days)
        naive_errors.append(naive_days - true_days)
        print(f"   {tank_index+1}   | {true_days:5.2f}  | {gauge_days:5.2f}   | {naive_days:5.2f}")
    print(f"  MAE: gauge {np.mean(np.abs(gauge_errors)):.2f} d   vs   calendar-days {np.mean(np.abs(naive_errors)):.2f} d")
